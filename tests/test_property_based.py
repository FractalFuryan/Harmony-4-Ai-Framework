...
from harmony.core.fractal_care_bot import FractalCareBot, Missy, Tag
...
SYMBOLIC_KEYWORDS = [
    ...
    "mandate",
    "must",
]  # Note: "must" here is symbolic context
...
def test_tag_classification_monotonic(user_input):
    ...
    if any(k in lower for k in EMOTIONAL_KEYWORDS):
        assert (
            Tag.EMOTIONAL in tags
        ), f"Missing EMOTIONAL tag for input containing emotional keywords: {user_input}"
...
def test_system_never_coercive_property(user_input):
    ...
    assume(
        user_input.lower()
        not in ["exit kat", "exit missy", "reset agent", "power down", "kat mode"]
    )
