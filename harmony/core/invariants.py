...
class EthicalInvariants:
    ...
    def check_consent_monotonicity(
        ...
        if not consent_granted and state_changed:
            self.violations.append("INV-1 VIOLATION: State changed without consent")
            return False
        return True
    ...
    def check_no_hidden_optimization(
        ...
        if hidden:
            self.violations.append(f"INV-3 VIOLATION: Hidden optimization detected: {hidden}")
            return False
        return True
    ...
    def check_drift_transparency(
        ...
        if drift > threshold:
            self.violations.append(
                f"INV-4 VIOLATION: Drift exceeds threshold " f"({drift:.4f} > {threshold:.4f})"
            )
            return False, drift
    ...
    def check_all_invariants(
        ...
        results = [
            ...
            self.check_boundary_preservation(boundary_before, boundary_after, consent_granted),
        ]

        if coherence_before is not None and coherence_after is not None:
            if not consent_granted:
                results.append(
                    self.check_refusal_without_penalty(coherence_before, coherence_after)
                )
