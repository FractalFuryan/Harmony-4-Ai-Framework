...
def create_phase_system(
    ...
    drift_detector = (
        PhaseDriftDetector(threshold=drift_threshold) if enable_drift_detection else None
    )
...
def create_guarded_role(
    ...
    boundary_guard = BoundaryGuard(min_integrity=min_boundary) if enable_boundary_guard else None
